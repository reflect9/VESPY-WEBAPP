Vespy.Node = function(p, page) {
	if(p) {
		...
		this.position = typeof p.position !== 'undefined' ? _.clone(p.position) : [1,1];
		this.type = typeof p.type !== 'undefined' ? _.clone(p.type) : undefined;
		this.executed = typeof p.executed !== 'undefined' ? _.clone(p.executed) : false;
		this.next = typeof p.next !== 'undefined' ? _.clone(p.next) : [];
		this.page = page;
	} else {
		this.I= ['_above','_left'];
		this.ID= makeid();
